...
function buildInstructions(spec: SolanaMintSpec, target: MintTarget, keypair: Keypair): TransactionInstruction[] {
...
/** Builds and signs a fresh transaction against the current blockhash. Called once up front, and again if a broadcast attempt fails with a stale-blockhash error. */
...
/**
 * KNOWN LIMITATION: only supports a single signer — the wallet passed in.
 * Some Solana mints (e.g. Candy Machine flows that create a fresh mint
 * account in the same instruction) require a second, ephemeral keypair to
 * co-sign. That's not handled here yet; see Tasks.md.
 */
export async function mintOnSolana(target: MintTarget, walletLabel: string): Promise<SolanaMintResult> {
  ...
  const instructions = buildInstructions(spec, target, keypair);
  ...
  for (let attempt = 0; attempt <= MAX_BLOCKHASH_RETRIES; attempt++) {
    const { txSignature, rawTx } = await buildSignedTx(primaryConnection, instructions, keypair);
    ...
    if (successes.length > 0) {
      ...
      return { signature: txSignature, wonVia };
    }
    ...
    if (staleBlockhash && attempt < MAX_BLOCKHASH_RETRIES) {
      ...
      continue; // loop rebuilds with a fresh blockhash
    }
...
export async function dryRunSolanaMint(target: MintTarget, walletLabel: string): Promise<DryRunResult> {
  ...
  let instructions;
  try {
    instructions = buildInstructions(spec, target, keypair);
  } catch (err) {
